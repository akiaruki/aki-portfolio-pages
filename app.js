...
function renderShowcase(){
 ...
 $('#featured').innerHTML='<p class="sr-only" id="showcase-help">Scroll horizontally, swipe, or use Left and Right Arrow keys to explore.</p><div class="showcase-track" role="region" aria-label="Selected photographs" aria-describedby="showcase-help" tabindex="0">'+showcase.map((p,i)=>
 '<figure class="featured-figure" role="group" aria-label="Photograph '+(i+1)+' of '+showcase.length+'"><div class="photo-button"><img src="'+escape(imageURL(p))+'" alt="'+escape(p.alt||PortfolioContent.photoTitle(p)||'Photograph')+'" draggable="false" '+(i===0?'fetchpriority="high"':'loading="lazy"')+' decoding="async"></div></figure>'
 ).join('')+'</div>';
 ...
 if(loopFrame){loopFrame.setAttribute('aria-hidden','true');loopFrame.querySelector('img').loading='eager';track.append(loopFrame);}
 ...
 function resetLoop(){track.scrollTo({left:0,behavior:'instant'});}
 track.addEventListener('scroll',()=>{if(loopFrame&&track.clientWidth&&Math.abs(track.scrollLeft-loopFrame.offsetLeft)<=2)resetLoop();},{passive:true});
 track.addEventListener('scrollend',()=>{if(loopFrame&&track.clientWidth&&track.scrollLeft>frames.at(-1).offsetLeft+track.clientWidth/2)resetLoop();});
 let showcaseVisible=true,resumeIndex=0,restoreFrame=0;
 setShowcaseVisible=visible=>{
  if(visible===showcaseVisible)return;
  cancelAnimationFrame(restoreFrame);
  if(!visible){resumeIndex=nearest();track.scrollTo({left:frames[resumeIndex].offsetLeft,behavior:'instant'});}
  showcaseVisible=visible;
  if(visible){
   const restore=()=>{track.scrollTo({left:frames[resumeIndex].offsetLeft,behavior:'instant'});scheduleShowcaseSize();track.dispatchEvent(new Event('showcase-resume'));};
   restore();restoreFrame=requestAnimationFrame(restore);
  }
 };
 function advanceShowcase(){if(loopFrame&&track.scrollLeft>frames.at(-1).offsetLeft+track.clientWidth/2){resetLoop();return;}const index=nearest();if(index===frames.length-1&&loopFrame)track.scrollTo({left:loopFrame.offsetLeft,behavior:motion()});else go(index+1);}
 ...
 function go(index,focus=false){index=Math.max(0,Math.min(frames.length-1,index));if(focus)track.focus({preventScroll:true});track.scrollTo({left:frames[index].offsetLeft,behavior:motion()});}
 if(frames.length>1){
 ...
 if(frames.length>1){
  ...
  function blocked(){return !showcaseVisible||paused||hovering||holding||focused||document.hidden||!track.getClientRects().length||!!document.querySelector('dialog[open]');}
  ...
  track.addEventListener('showcase-resume',()=>{holding=false;hovering=matchMedia('(hover:hover)').matches&&host.matches(':hover');focused=host.contains(document.activeElement)&&document.activeElement.matches(':focus-visible');schedule();});document.addEventListener('visibilitychange',()=>{holding=false;schedule();});window.addEventListener('hashchange',schedule);
...
function renderOverview(){
 ...
 if(view!=='home')setShowcaseVisible(false);$('.opening').hidden=view!=='home';if(view==='home')setShowcaseVisible(true);$('.about').hidden=view!=='home';$('#photographs').hidden=!['home','posts'].includes(view);$('#collections').hidden=view!=='collections';$('#journal').hidden=view!=='journal';
 ...
 if(mode!==renderedMode){
  ...
  $('#collection-wall').innerHTML=(allCollections?collections:collections.slice(0,3)).map((collection,i)=>{const collectionPhotos=collection.photoIds.map(id=>photoMap.get(id)).filter(Boolean);return '<article class="collection-preview reveal" data-collection-id="'+escape(collection.id)+'"><div class="collection-frame collection-grid" data-count="'+collectionPhotos.length+'" role="group" aria-label="'+escape(collection.title)+'">'+collectionPhotos.map(p=>'<button type="button" class="collection-photo-frame" data-photo="'+photos.indexOf(p)+'" aria-label="View '+escape(PortfolioContent.photoTitle(p)||'photograph')+'"><img src="'+escape(imageURL(p))+'" alt="'+escape(p.alt||PortfolioContent.photoTitle(p)||'Photograph')+'" loading="lazy" decoding="async"></button>').join('')+'</div><div class="collection-caption"><div><p class="eyebrow">Collection '+String(i+1).padStart(2,'0')+' '+sampleLabel(collection)+'</p><h3>'+escape(collection.title)+'</h3>'+(collection.subtitle?'<p>'+escape(collection.subtitle)+'</p>':'')+'</div></div></article>';}).join('')||'<p class="quiet-empty">A space for photographs that belong together.</p>';
...
function showPhoto(index){if(!viewerPhotos.length)return;active=(index+viewerPhotos.length)%viewerPhotos.length;const p=viewerPhotos[active];$('#lightbox-image').src=imageURL(p);$('#lightbox-image').alt=p.alt||PortfolioContent.photoTitle(p)||'Photograph';const photoTitle=PortfolioContent.photoTitle(p);$('#lightbox-title').textContent=photoTitle;$('#lightbox-title').hidden=!photoTitle;if(photoTitle){dialog.setAttribute('aria-labelledby','lightbox-title');dialog.removeAttribute('aria-label');}else{dialog.removeAttribute('aria-labelledby');dialog.setAttribute('aria-label','Image viewer');}$('#lightbox-meta').textContent=[p.place,p.sample?'Sample photograph':p.capturedOn?formatDate(p.capturedOn):p.date?[p.date,p.year].filter(Boolean).join(' '):''].filter(Boolean).join(' · ');$('#lightbox-story').textContent=p.story;$('#lightbox-story').hidden=!p.story;$('#lightbox-count').textContent=`${String(active+1).padStart(2,'0')} / ${String(viewerPhotos.length).padStart(2,'0')}`;$('#lightbox-credit').innerHTML=p.credit?`Photograph by ${p.source?`<a href="${escape(safeURL(p.source))}" target="_blank" rel="noopener noreferrer">${escape(p.credit)} ↗</a>`:escape(p.credit)}`:'';$('#lightbox-meta').hidden=!$('#lightbox-meta').textContent;$('#lightbox-title').parentElement.hidden=!photoTitle&&!$('#lightbox-meta').textContent&&!p.story;$('.previous').hidden=$('.next').hidden=viewerPhotos.length<2;dialog.scrollTop=0;}
document.addEventListener('click',event=>{const button=event.target.closest('[data-photo]');if(!button)return;const p=photos[Number(button.dataset.photo)];const feedPost=button.closest('[data-post-id]'),collection=button.closest('[data-collection-id]');viewerPhotos=collection?data.collections.find(item=>item.id===collection.dataset.collectionId).photoIds.map(id=>photoMap.get(id)):button.closest('#story-view')?storyPhotos:feedPost?data.posts.find(post=>post.id===feedPost.dataset.postId).photoIds.map(id=>photoMap.get(id)):showcase;if(!viewerPhotos.includes(p))viewerPhotos=[p];previousFocus=button;showPhoto(viewerPhotos.indexOf(p));dialog.showModal()});
